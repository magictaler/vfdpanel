...
MVFD_16S8D::MVFD_16S8D(uint8_t cs, uint8_t sclk, uint8_t data)
{
...
void MVFD_16S8D::write(uint8_t value) 
{
...
uint8_t MVFD_16S8D::dispProductName()
{
    if (_testBt != 0)
    {
        ...
        {
            driver.addrSetCmd(0);
            print_p(PSTR("16 SEGMN"));
        }else if (_testBt == VFD_MAX_TEST_DELAY * 1 / 20)
        {
            driver.addrSetCmd(0);
            print_p(PSTR("8 DIGITS"));
        }
        _testBt--;
        return true;
    }
...
uint8_t MVFD_16S8D::scrollStep( void )
{
    char c;
    uint8_t idx = 0;
    if(!scrollPntr) return false;

    prog_char* str = scrollPntr;
    ...
    if (!pgm_read_byte(scrollPntr)) return false;
    else return true;
}
